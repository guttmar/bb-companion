...
const normalizeSkillDescription = (description: string): string => {
  ...
  },
  {
    ...
    skills: [
      ...
      skill(
        ...
        "Ball and Chain",
        "active",
        "When this player is activated, the only action they can announce is a special Ball and Chain action; there is no limit to the number of players who can announce this special action per turn. When a player performs a Ball and Chain special action, place the return template on that player, facing one of the end zones or one of the touchlines, then roll a D6 and move that player to the square indicated by the template. A player who moves in this way does not have to make an agility test to dodge out of the tackle zone; they automatically succeed. Opposing players cannot use the Pursuit or Tentacles skills against a player performing a Ball and Chain action. If this movement takes the player off the field, they risk being injured by the crowd. If this movement takes the player to a square occupied by a standing player (regardless of team), they make a block against that player; this blocking action ignores the Repulsion skill. If it is a teammate, then that player's coach chooses the result to apply. If this move takes the player to a square occupied by a prone or stunned player, that player is pushed back and an armor roll is made against them. If this move takes the player to a square containing the ball, the ball immediately bounces. This does not cause a turnover. A player who performs a special Ball and chain action can move a number of square equal to or less than their movement. They can go for it normally, but if they roll a 1, they first move to the square by blocking, pushing a player, or bouncing the ball, if applicable, before falling onto the square they moved to. If this player is tackled, falls, or is knocked down for any reason, immediately roll for injury, treating any Stunned result as KO instead. A player with this trait cannot have any of the following skills: Diving Tackle, Eye Gouged, Frenzy, Grab, Hit-and-Run, Leap, Multiple Block, On the Ball, Shadowing, Steady Footing."
      ),
      skill(
        "bloodlust",
        "Bloodlust (x+)",
